...
static void FGTestDisconnectOutcomes(void) { FGChallengeCoordinator *c = FGCoordinator(NULL); NSDate *disconnect = [NSDate dateWithTimeIntervalSince1970:300]; NSDictionary<NSString *, id> *local; id<FGChallengeRaceSceneEventDelegate> sink; FGRequire(FGCoordinatorPrepareRace(c, FGCoordinatorContract(@"forfeit")) && [c recordPeerDisconnectedAtDate:disconnect] && [c advanceToDate:[disconnect dateByAddingTimeInterval:5.0]], @"reconnect deadline is processed as forfeit"); FGRequire(c.state == FGChallengeCoordinatorStateVerifying && c.outcome == FGChallengeOutcomeWin, @"peer timeout forfeits"); local = FGCoordinatorFinalRecord(@"forfeit", @"player-alpha", 1, 1, NO, 0); sink = (id<FGChallengeRaceSceneEventDelegate>)c; [sink challengeRaceScene:nil didProduceLocalFinalRecord:local]; FGRequire([c completeVerificationWithLocalFinalRecord:local remoteFinalRecord:FGCoordinatorFinalRecord(@"forfeit", @"player-bravo", 9, 9, NO, 0) remoteDerivedOutcome:FGChallengeOutcomeWin] && c.state == FGChallengeCoordinatorStateResults && c.outcome == FGChallengeOutcomeWin, @"contradictory records cannot replace grace forfeit"); c = FGCoordinator(NULL); FGRequire(FGCoordinatorPrepareRace(c, FGCoordinatorContract(@"void")) && [c recordLocalDisconnectedAtDate:disconnect] && [c recordPeerDisconnectedAtDate:disconnect], @"both disconnect accepted"); FGRequire(c.state == FGChallengeCoordinatorStateVoided && c.outcome == FGChallengeOutcomeVoid, @"both disconnect voids"); }
...
static void FGTestSceneFinalAuthority(void) { FGChallengeCoordinator *c = FGCoordinator(NULL); NSDictionary<NSString *, id> *accepted = FGCoordinatorFinalRecord(@"final-authority", @"player-alpha", 7, 5, NO, 0); NSDictionary<NSString *, id> *replacement = FGCoordinatorFinalRecord(@"final-authority", @"player-alpha", 8, 6, NO, 0); NSDictionary<NSString *, id> *remote = FGCoordinatorFinalRecord(@"final-authority", @"player-bravo", 6, 4, NO, 0); id<FGChallengeRaceSceneEventDelegate> sink; NSDate *crash = [NSDate dateWithTimeIntervalSince1970:600]; FGRequire(FGCoordinatorPrepareRace(c, FGCoordinatorContract(@"final-authority")) && [c recordLocalCrashAtDate:crash] && [c advanceToDate:[crash dateByAddingTimeInterval:3]], @"scene final verification reaches gate"); FGRequire(![c completeVerificationWithLocalFinalRecord:accepted remoteFinalRecord:remote remoteDerivedOutcome:FGChallengeOutcomeLoss], @"verification rejects absent accepted final state"); sink = (id<FGChallengeRaceSceneEventDelegate>)c; [sink challengeRaceScene:nil didProduceLocalFinalRecord:accepted]; [sink challengeRaceScene:nil didProduceLocalFinalRecord:replacement]; FGRequire([c.latestLocalFinalRecord isEqualToDictionary:accepted], @"first accepted scene final is immutable"); FGRequire(![c completeVerificationWithLocalFinalRecord:replacement remoteFinalRecord:remote remoteDerivedOutcome:FGChallengeOutcomeLoss], @"caller local final cannot replace accepted facts"); FGRequire([c completeVerificationWithLocalFinalRecord:nil remoteFinalRecord:remote remoteDerivedOutcome:FGChallengeOutcomeLoss] && c.outcome == FGChallengeOutcomeWin && c.isResultVerified, @"verification uses accepted final facts"); }
int main(void) { @autoreleasepool { FGTestReadiness(); FGTestMismatch(); FGTestWindowAndGrace(); FGTestDisconnectOutcomes(); FGTestBoundaryAndTransportUnavailable(); FGTestDisagreementAndRematch(); FGTestSceneEventIntake(); FGTestSceneFinalAuthority(); puts("PASS: Live Challenge coordinator"); } return 0; }
